Shader "RTCube_Demo/SCRT_DemoHDR_Ref2" {
	Properties {
	...
	SubShader {
		...
		void surf (Input IN, inout SurfaceOutput o) {
			...
			float d = clamp(pow(f,1.5),0.15,1);
			f = (1-f);
			float f1 = clamp(pow(f,2),0.05,1);
			float f2 = clamp(pow(f,2),0.1,1);
			c1 *= SC_texCUBERGBM(_Cube, IN.worldRefl,_Max,1,0) * f1;
			c2 *= SC_texCUBERGBM(_Cube, IN.worldRefl,_Max,1,3) * f2;
			c3 *= SC_texCUBERGBM(_Cube, IN.worldRefl,_Max,1,4) * d;
